...
const RATING_VARIANTS: Record<RatingVariant, { filled: string; empty: string }> = {
  primary: { filled: 'text-blue-500', empty: 'text-slate-300' },
  warning: { filled: 'text-amber-400', empty: 'text-slate-300' },
  danger: { filled: 'text-rose-500', empty: 'text-slate-300' },
};
...
export class TwRatingComponent implements ControlValueAccessor {
  ...
  protected valueClasses() {
    return 'ml-2 text-sm font-medium text-slate-600';
  }
